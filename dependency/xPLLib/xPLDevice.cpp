...
void xPLDevice::Initialisation(const std::string& vendor, const std::string& device, const std::string& instance)
{
    ...
    m_pSenderSock = &m_DefaultSenderSock;
    m_pReceiverSock = &m_DefaultReceiverSock;
...
void xPLDevice::SetSockets(ISimpleSockUDP* senderSock, ISimpleSockUDP* receiverSock)
{
    if(senderSock==nullptr)
        m_pSenderSock = &m_DefaultSenderSock;
    else
        m_pSenderSock = senderSock;

    if(receiverSock==nullptr)
        m_pReceiverSock = &m_DefaultReceiverSock;
    else
        m_pReceiverSock = receiverSock;
}
...
void xPLDevice::SetInstance(const string& instance)
{
    ...
    if(m_pSenderSock->isOpen()) SendHeartBeatEnd();
    try
    {
    ...
    if(m_pSenderSock->isOpen()) SendMessage(m_HBeatMsg, "*");
...
void xPLDevice::SetHeartBeat(HeartBeatType type, int interval)
{
    ...
    switch(type)
    {
        ...
        case xPLDevice::ConfigAPP :
            m_HBeatMsg = new SchemaConfigApp(interval, m_pReceiverSock->GetPort(), m_pReceiverSock->LocalAddress(""));
            break;
        ...
        case xPLDevice::HeartBeatAPP :
            m_HBeatMsg = new SchemaHbeatApp(interval, m_pReceiverSock->GetPort(), m_pReceiverSock->LocalAddress(""));
            break;
...
{
    return m_pReceiverSock->GetPort();
}
...
void xPLDevice::DiscoverTCPPort()
{
	...
	LOG_ENTER;
    m_pReceiverSock->SetNetworkInterface(m_networkInterface);
    m_pReceiverSock->Blocking(false);

    try
    {
        m_pReceiverSock->Listen(portHub, INADDR_ANY);
    ...
    for(portTCP=portMin; portTCP <= portMax; portTCP++)
    {
        try
        {
            m_pReceiverSock->Listen(portTCP, INADDR_ANY); //"127.0.0.1"); //"192.168.0.84"); //INADDR_ANY);
...
bool xPLDevice::WaitRecv(int delay)
{
    ...
    if(!m_pReceiverSock->WaitRecv(delay))
    {
        LOG_TRACE(m_Log) << "Nothing to receive";
        return false;
    }
    if(!m_pReceiverSock->Recv(xPLRaw))
    {
